...
$DOCKER_PATH run --rm -i --name $DOCKER_IMAGE -e IIGB_BUILD=$IIGB_BUILD -e BUCKET=$BUCKET -e DID=$DID -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY -e NO_ROBOTS=$NO_ROBOTS -e NO_CACHE=$NO_CACHE $DOCKER_IMAGE /bin/bash -c "npm config set great.gov.uk:cms_branch $CMS_BRANCH && npm run deploy"
